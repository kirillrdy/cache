...
//! function and every top-level declaration it transitively references, and
//! reports any purity violation it can see.
//!
...
/// Namespaces reachable from a pure function, as `<import>.<first field>`.
/// Allowlisting the import alone would be useless in Zig, where everything
/// arrives through `std`.
const pure_namespaces = [_][]const u8{
    "std.math", "std.mem",     "std.ascii", "std.sort",
    "std.fmt",  "std.unicode", "std.meta",  "std.simd",
};

const pure_builtins = [_][]const u8{
    "@as",        "@intCast",      "@floatFromInt",       "@intFromFloat",
    "@truncate",  "@min",          "@max",                "@abs",
    "@sqrt",      "@sizeOf",       "@alignOf",            "@TypeOf",
    "@typeInfo",  "@typeName",     "@enumFromInt",        "@intFromEnum",
    "@field",     "@hasDecl",      "@hasField",           "@splat",
    "@reduce",    "@compileError", "@panic",              "@bitCast",
    "@errorName", "@This",         "@memcpy",             "@divFloor",
    "@divTrunc",  "@mod",          "@rem",                "@popCount",
    "@clz",       "@ctz",          "@byteSwap",           "@bitReverse",
    "@floatCast", "@boolToInt",    "@setEvalBranchQuota",
};
...
fn checkBody(tree: Ast, decls: *const DeclMap, root: []const u8, d: Decl, report: *Report) !void {
    ...
    while (i <= last) : (i += 1) {
        switch (tree.tokenTag(i)) {
            .builtin => {
                const b = tree.tokenSlice(i);
                if (std.mem.eql(u8, b, "@import")) continue;
                if (!containsStr(&pure_builtins, b)) {
                    try report.err("{s}: reaches {s} via {s}, which is not on the pure-builtin list", .{ root, b, d.name });
                }
            },
            .identifier => {
                const s = tree.tokenSlice(i);
                ...
                if (ref.is_import and i + 2 <= last and tree.tokenTag(i + 1) == .period and
                    tree.tokenTag(i + 2) == .identifier)
                {
                    const ns = tree.tokenSlice(i + 2);
                    var ok = false;
                    for (pure_namespaces) |allowed| {
                        var parts = std.mem.splitScalar(u8, allowed, '.');
                        const a_pkg = parts.next().?;
                        const a_ns = parts.next().?;
                        if (std.mem.eql(u8, a_pkg, s) and std.mem.eql(u8, a_ns, ns)) ok = true;
                    }
                    if (!ok) {
                        try report.err("{s}: reaches {s}.{s} via {s}, which is not on the pure-namespace list", .{ root, s, ns, d.name });
                    }
                }
            },
            .keyword_asm => try report.err("{s}: contains inline asm via {s}", .{ root, d.name }),
            .keyword_volatile => try report.err("{s}: volatile access via {s}", .{ root, d.name }),
            else => {},
        }
    }
